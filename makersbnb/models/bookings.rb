class Bookings < ActiveRecord::Base
    ...
    def self.must_be_avaliable(start_date, end_date, space_id)
        results = self.connection.execute("SELECT * FROM availabilities WHERE start_date <= DATE '#{start_date}' AND end_date >= DATE '#{end_date}' AND spaces_id = '#{space_id}' ")
        if results.to_a.empty?
            return false
        else
            return true
    ...
    def self.valid?(start_date,end_date,space_id)
        Bookings.must_be_avaliable(start_date,end_date,space_id) && Bookings.must_not_overlap(start_date,end_date)
    end
